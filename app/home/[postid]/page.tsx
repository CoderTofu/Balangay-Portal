...
export default function PostDetail() {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      setCurrentUserId(window.localStorage.getItem("curruser_id"));
    }, 0);
    ...
    try {
      const fetchListings = async () => {
        const res = await fetch(
          "http://localhost:8080/api/trades/get-all-open-trades",
        );
  ...
  const isOwnListing = !!currentUserId && listing?.author_id === currentUserId;
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24">
      ...
      <div className="mx-auto w-full max-w-162.5 space-y-4 px-4 pt-6">
        <div className="rounded-2xl bg-white shadow-[0_10px_30px_rgba(16,24,40,0.10)]">
          ...
          <div className="px-5 pb-6 pt-5">
              ...
              <div className="flex justify-center items-center gap-4">
                <div>
                  ...
                  <div className="text-[#17136D] text-center">
                    <StarRating value={clamp(ratingAvg, 0, 5)} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 border-t border-slate-400 pt-5">
              ...
              <div className="mt-2">
                {listing?.requests?.map((request: string) => (
                  <span
                    key={request}
                    className="inline-flex rounded-lg bg-[#D3E4FE] px-3 py-1.5 text-xs font-bold tracking-wide text-[#17136D]"
                  >
                    {request}
                  </span>
                ))}
